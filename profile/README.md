...
### 담당
- 장바구니
    - 비회원과 회원을 DB에 저장하기 위해 장바구니 테이블을 따로 만들어 비회원은 cartId를 쿠키를 통해 관리하여 비회원과 회원 장바구니를 영구적으로 저장할 수 있게 개선했습니다.
    - 고객들이 장바구니를 빠르게 조회할 수 있도록 DB와 Redis를 동기화하여 처리하였고, 장바구니를 조회할 때는 TPS를 전보다 50% 증가시켰습니다.
    - 비회원, 회원 장바구니 REST API를 설계하고 UI를 구현하였습니다.
- 주문
    - Toss Payments에 오류가 나면 모든 작업을 롤백하기 위해 포인트, 쿠폰, 주문 처리를 한 트랜잭션에 묶어서 구현하여 결제 오류 시에 포인트, 쿠폰이 없어지는 오류를 막았습니다.
    - 정확한 주소를 고객에게 제공하기 위해 행안부 주소도로명 API를 추가하여 배송지를 정확한 주소로 추가할수 있게 구현하였습니다.
    - 회원, 비회원 주문 프로세스 REST API를 설계하고 UI를 구현하였습니다.
- 쿠폰
    - 쿠폰 발급이 몰릴 시에 오류가 나서, 쿠폰 발급에 비동기 메시지 큐인 RabbitMQ를 적용하여 오류를 0%로 줄이고 TPS도 10% 증가시켰습니다.
    - 북스토어 서버와의 의존성을 줄이기 위해 쿠폰 서버와 DB를 따로 만들어 북스토어 서버가 오류가 나도 쿠폰 발급은 정상적으로 구동되게 구현하였습니다.
    - 쿠폰 만료를 사용자들에게 알리기 위해 배치 서버와 RabbitMQ를 사용하여 비동기적으로 메시지 알림을 구현하여 오류를 0%로 줄일 수 있었습니다.
    - 쿠폰 REST API 설계와 UI 구현하였습니다.
- 포인트
    - 포인트 REST API 구축과 UI 구현하였습니다.
    - 포인트 정책 REST API 구축과 UI 구현하였습니다.
- 인프라
    - nginx에 https추가하고 전체적인 어플리케이션, Docker 네트워크 설계
